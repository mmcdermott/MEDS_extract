...
Earlier iterations split this across ``source.py``, ``fetcher.py``, and a
``_types.py`` to break import cycles caused by ``Source`` constructing a
``Fetcher`` as its default. With ``Fetcher`` gone, no cycle remains; one file
holds the type definitions, the ABC, and the orchestration.
