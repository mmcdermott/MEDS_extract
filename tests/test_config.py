...
# ── _metadata blocks: validation-at-construction (dftly program over the metadata table) ──


def test_metadata_block_validated_at_messy_parse_time():
    """``_metadata`` config mistakes fail ``MessyConfig.parse`` — before any stage runs.

    The zero-join-key case: none of the produced columns name a code component, so the
    error surfaces at config load (naming event, prefix, and the available components),
    not mid-pipeline in ``extract_code_metadata``.
    """
    with pytest.raises(ValueError, match=r"event 'chart', metadata prefix 'd_items'.*\['itemid'\]"):
        MessyConfig.parse(
            {
                "chartevents": {
                    "chart": {
...
def test_metadata_block_on_literal_code_fails_at_parse_time():
    """A ``_metadata`` block on a literal code is rejected at construction."""
    with pytest.raises(ValueError, match="literal code has no components to match metadata on"):
        MessyConfig.parse(
            {
                "admissions": {
                    "admit": {
...
def test_metadata_block_match_on_fails_at_parse_time():
    """A leftover pre-0.7 ``_match_on`` key is rejected at construction with a migration pointer."""
    with pytest.raises(ValueError, match=r"_match_on.*removed in 0\.7\.0"):
        EventConfig.parse(
            "med",
            {
                "code": 'f"{$medication_name}//{$dose}"',
                "time": None,
                "_metadata": {"med_classes": {"_match_on": "medication_name", "description": "$drug_class"}},
            },
        )


def test_metadata_block_reserved_output_name_fails_at_parse_time():
    """Producing ``code_template`` (always) or a non-component ``code`` is rejected at construction."""
    with pytest.raises(ValueError, match=r"\['code_template'\] are reserved"):
        EventConfig.parse(
            "chart",
            {
                "code": 'f"CHART//{$itemid}"',
...
def test_metadata_block_key_named_code_is_allowed_for_code_component():
    """A key named ``code`` is legal when the code expression references a column named ``code``.

    The idiomatic ICD/OMOP vocabulary shape: the key is consumed by the component join
    and never collides with the pipeline-generated output ``code``.
    """
    ...
    assert ev.metadata == {"icd_meta": {"code": "$code", "description": "$long_title"}}


def test_metadata_block_list_value_fails_with_coalesce_pointer():
    """The pre-0.7 list-coalesce shorthand is rejected with the dftly ``coalesce`` rewrite."""
    with pytest.raises(ValueError, match=r"coalesce\(\$special_title, \$title\)"):
        EventConfig.parse(
            "lab",
            {
                "code": "$test_name",
...
def test_metadata_block_unparsable_expression_names_column():
    """A value that fails dftly parsing is rejected naming the offending output column."""
    with pytest.raises(ValueError, match=r"_metadata column 'description'.*failed to parse"):
        EventConfig.parse(
            "lab",
            {
                "code": "$test_name",
                "time": None,
                "_metadata": {"lab_meta": {"test_name": "$test_name", "description": "LOINC/{label}"}},
            },
        )
