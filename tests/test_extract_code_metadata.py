...
def test_zero_join_key_metadata_block_fails_the_stage():
    """A block producing no component-named columns errors at config load, in the stage run.

    The error must list the component columns the event offers so the fix is obvious.
    """
    messy = """\
chartevents:
  chart:
...
"""
    with (
        ...
        pytest.raises(ValueError, match=r"produces no join-key columns.*\['itemid'\]"),
    ):
        _run_ecm_scenario(
            Path(d),
            messy,
...
def test_match_on_gets_targeted_migration_error():
    """A leftover pre-0.7 ``_match_on`` key fails the stage with a migration pointer."""
    messy = """\
medications:
  med:
...
"""
    with (
        tempfile.TemporaryDirectory() as d,
        pytest.raises(ValueError, match=r"_match_on.*removed in 0\.7\.0.*MIGRATION\.md"),
    ):
        _run_ecm_scenario(
            Path(d),
            messy,
            event_frames={
                "medications": pl.DataFrame(
                    {
                        "code": ["Metformin//500 mg"],
                        "code_components": [{"medication_name": "Metformin", "dose": "500 mg"}],
                        "source_block": ["medications/med"],
                    }
                )
            },
            raw_files={"med_classes.csv": "medication_name,drug_class\nMetformin,Antidiabetic\n"},
        )
