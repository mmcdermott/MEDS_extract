...
Not a test module. The ``_`` prefix keeps ``pytest --collect-only`` from matching it
as a test file; the ``if __name__ == "__main__"`` guard below keeps it safe against
the ``--doctest-modules`` addopts pass, which would otherwise execute the body at
import time.

Why a subprocess test at all: we need real SIGINT semantics — sending a signal to a
live ``download_all`` call and observing exit behavior. pytest itself catches
``KeyboardInterrupt`` and stops the session, so there's no way to observe "does the
download exit quickly?" from inside a pytest worker. A subprocess running
``Source.download_all`` with a stub source is the cleanest isolation.
...
* With the ``shutdown(wait=False, cancel_futures=True)`` fix, only the in-flight
  batch (~``max_workers`` files + a small race margin) completes before
...
Usage: ``python tests/_fetcher_sigint_child.py <dest_dir>`` — exits 0 on success
(i.e. ``KeyboardInterrupt`` was caught cleanly), 99 on "download_all completed
despite SIGINT" (the unexpected success that would indicate the fix regressed).
"""
...
# 100 files, 0.2 s per-file sleep — serial drain at the default concurrency of 4 would
# take ~5 s. SIGINT fires at 0.15 s, long before all 100 submissions could complete.
_N_FILES = 100
...
def main() -> int:
    ...
    try:
        # No pool passed → ``download_all`` builds a private pool with the
        # SIGINT-safe ``cancel_futures`` shutdown, which is the path under test.
